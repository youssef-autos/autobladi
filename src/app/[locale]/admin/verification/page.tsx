...
import { listPendingVerifications } from "@/lib/queries/admin"
...
export default async function AdminVerificationPage({
  ...
  const t = await getTranslations("adminPanel.verificationQueue")
            ...
            <table className="w-full">
              <thead className="bg-moroccan-sand-50/60 text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                <tr>
                  ...
                  <th className="text-start px-4 py-3">{t("columns.company")}</th>
                  <th className="text-start px-4 py-3">{t("columns.rc")}</th>
                  <th className="text-start px-4 py-3">{t("columns.documents")}</th>
