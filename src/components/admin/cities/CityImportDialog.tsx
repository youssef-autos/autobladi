...
import { bulkImportCities } from "@/app/[locale]/admin/cities/actions"
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
import { cityImportItemSchema, type CityImportItem } from "@/lib/validations/city"
...
export function CityImportDialog({ open, onOpenChange }: Props) {
  const t = useTranslations("adminPanel.citiesPage")
  const tUp = useTranslations("adminPanel.citiesPage.upload")
  const fileInputRef = useRef<HTMLInputElement>(null)
  const [items, setItems] = useState<CityImportItem[] | null>(null)
  ...
  function handleFile(file: File) {
    ...
    reader.onload = () => {
      try {
        ...
        const parsed: CityImportItem[] = []
        for (let i = 0; i < raw.length; i++) {
          const result = cityImportItemSchema.safeParse(raw[i])
          if (!result.success) {
            setError(
              tUp("invalidEntry", {
                index: i + 1,
  ...
  function onImport() {
    ...
    startTransition(async () => {
      const res = await bulkImportCities(items)
      if (!res.ok) {
            ...
            <div className="max-h-64 overflow-y-auto text-xs">
              <table className="w-full">
                <thead className="bg-moroccan-sand-50/30 text-muted-foreground">
                  <tr>
                    <th className="text-start px-3 py-1.5">name_ar</th>
                    <th className="text-start px-3 py-1.5">name_fr</th>
                    <th className="text-start px-3 py-1.5">slug</th>
                    <th className="text-start px-3 py-1.5">region</th>
                  </tr>
                </thead>
                <tbody>
                  {items.slice(0, 50).map((c, idx) => (
                    <tr key={idx} className="border-t border-border">
                      <td className="px-3 py-1.5">{c.name_ar}</td>
                      <td className="px-3 py-1.5">{c.name_fr}</td>
