...
import { bulkImportModels } from "@/app/[locale]/admin/models/actions"
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export function ModelImportDialog({ open, onOpenChange }: Props) {
  const t = useTranslations("adminPanel.modelsPage")
  const tUp = useTranslations("adminPanel.modelsPage.upload")
  const fileInputRef = useRef<HTMLInputElement>(null)
  const [items, setItems] = useState<ModelImportItem[] | null>(null)
  ...
  function handleFile(file: File) {
    ...
    reader.onload = () => {
      try {
        ...
        const parsed: ModelImportItem[] = []
        for (let i = 0; i < raw.length; i++) {
          const result = modelImportItemSchema.safeParse(raw[i])
          if (!result.success) {
            setError(
              tUp("invalidEntry", {
                index: i + 1,
  ...
  function onImport() {
    ...
    startTransition(async () => {
      const res = await bulkImportModels(items)
      if (!res.ok) {
      ...
      toast.success(tUp("summary", { imported, updated, skipped }))
      setItems(null)
      setFileName(null)
      onOpenChange(false)
    })
            ...
            <div className="max-h-64 overflow-y-auto text-xs">
              <table className="w-full">
                <thead className="bg-moroccan-sand-50/30 text-muted-foreground">
                  <tr>
                    <th className="text-start px-3 py-1.5">brand_slug</th>
                ...
                <tbody>
                  {items.slice(0, 50).map((m, idx) => (
                    <tr key={idx} className="border-t border-border">
                      <td className="px-3 py-1.5 font-mono text-moroccan-red-500">
                        {m.brand_slug}
                      </td>
